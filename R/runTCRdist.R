...
#'   scRepertoire data and calculates pairwise TCR distances using tcrdist3.
#'
...
#' @param add_to_object Logical. If TRUE, attempts to add distance matrix to Seurat object
#'   (stored in misc slot). Default is FALSE due to large matrix size.
#'
...
#'   If add_to_object=TRUE, returns the Seurat object with distances in obj@misc$tcrdist
#'
#' @export
...
#' }
runTCRdist <- function(seurat_obj,
                       chains = "beta",
                       organism = "human",
  ...
  for (chain in chains) {
    ...
    chain_data <- immApex::getIR(seurat_obj, chains = immapex_chain)
  ...
  if (add_to_object) {
    seurat_obj@misc$tcrdist <- output
    message("TCR distances added to Seurat object at obj@misc$tcrdist")
    return(seurat_obj)
  } else {
    return(output)
  }
