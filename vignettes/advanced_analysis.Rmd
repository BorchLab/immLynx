...
seurat_mcl_2 <- runClustTCR(immLynx_example, inflation = 2.0,
                            column_prefix = "mcl_2")
seurat_mcl_3 <- runClustTCR(seurat_mcl_2, inflation = 3.0,
                            column_prefix = "mcl_3")

# Compare number of clusters
cat("MCL (inflation=2):",
    length(unique(na.omit(seurat_mcl_3$mcl_2_TRB))),
    "clusters\n")
cat("MCL (inflation=3):",
    length(unique(na.omit(seurat_mcl_3$mcl_3_TRB))),
    "clusters\n")
```
...
ggplot(seurat_pgen[[]], aes(x = Type, y = olga_pgen_log10_TRB)) +
  geom_boxplot() +
